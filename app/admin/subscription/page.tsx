'use client'
/**
 * /admin/subscription — revenue + user growth at a glance.
 *
 * Was the old `/admin` (Dashboard) page. Renamed and moved here so the
 * sidebar's top-level "Dashboard" entry can point at the broader site
 * analytics surface, and revenue/conversion lives under its own route
...
export default function AdminSubscription() {
  ...
  if (isInitialLoading) {
    return <SubscriptionSkeleton dark={dark} />
  }
  if (error && !stats) {
    return <p className="text-red-400">Failed to load stats: {error.message}</p>
  }
  if (!stats) return <SubscriptionSkeleton dark={dark} />
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-[26px] font-bold tracking-tight" style={{ color: dark ? '#fff' : '#0f172a' }}>Subscription</h1>
          <p className="text-sm mt-1" style={{ color: dark ? '#64748b' : '#94a3b8' }}>Revenue, plans, and recent customer activity</p>
        </div>
...
// ── Subscription skeleton ── shown only on first visit; after that
// the shared admin cache makes revisits instant.
function SubscriptionSkeleton({ dark }: { dark: boolean }) {
  const cardBg = dark ? 'rgba(255,255,255,0.03)' : '#fff'
  const cardBorder = dark ? 'rgba(255,255,255,0.06)' : '#e2e8f0'
